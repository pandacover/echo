...
import { deleteNote, fetchLibrary, fetchNote } from '~/lib/notes.functions'

export const Route = createFileRoute('/notes/$noteId')({
  loader: async ({ params, context }) => {
    if (!context.userId) {
      return { note: null, wordCount: 0 }
    }
    const [note, library] = await Promise.all([
      fetchNote({ data: { id: params.noteId } }),
      fetchLibrary().catch(() => ({ dictionary: [] })),
    ])
    return { note, wordCount: library.dictionary.length }
  },
  component: NoteDetailPage,
})

function NoteDetailPage() {
  const { note, wordCount } = Route.useLoaderData()
  ...
  if (!note) {
    return (
      <AppShell wordCount={wordCount}>
        <div className="px-6 pt-10 text-nota-muted">Sign in to read this note.</div>
      </AppShell>
    )
  }
